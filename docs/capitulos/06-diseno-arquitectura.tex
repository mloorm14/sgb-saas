...
\texttt{diagrama\_c4\_capa2.jpeg}), reproducidos en la
\autoref{fig:c4-entrega1a-capa1} y la \autoref{fig:c4-entrega1a-capa2}
únicamente como referencia histórica de ese diseño ya superado -- nótese
que ambos difieren del modelo vigente descrito en las secciones
siguientes (por ejemplo, citan \texttt{Google Books API} como sistema
externo, retirado del modelo actual por no existir esa integración en
el código real, y el segundo no incluye el contenedor Redis,
incorporado después).

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{diagramas/diagrama_c4_capa1.png}
\caption{Diagrama estático de contexto del sistema, Entrega~1A (sin
fuente versionada) -- reemplazado por el Nivel~1 del modelo C4 vigente
en \texttt{workspace.dsl}, descrito en la \autoref{sec:c4-nivel1}.}
\label{fig:c4-entrega1a-capa1}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{diagramas/diagrama_c4_capa2.jpeg}
\caption{Diagrama estático de contenedores del sistema, Entrega~1A (sin
fuente versionada) -- reemplazado por el Nivel~2 del modelo C4 vigente
en \texttt{workspace.dsl}, descrito en la \autoref{sec:c4-nivel2}.}
\label{fig:c4-entrega1a-capa2}
\end{figure}
...
al DSL fuente y reproducido en la \autoref{fig:c4-componentes}.
