...
El modelo arquitectónico de SGB-SaaS está versionado como código fuente
...
la implementación actual que vale la pena señalar aquí: Gemini~2.0~Flash
...
"sistema externo" del Nivel~1 -- ver Nivel~3 más abajo). Tampoco se
...
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Frontend Angular} (TypeScript/Angular~17) -- SPA de
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Integración externa} (1 componente): \texttt{GeminiClient}
    -- cliente HTTP hacia la API de Gemini~2.0~Flash, con reintento
...
no, por lo que una validación histórica contra esa imagen no habría sido
una validación real. El diagrama de Nivel~3 se exportó a SVG mediante el
flujo \texttt{structurizr/structurizr export -format plantuml}
seguido de renderizado con PlantUML, resultando en
\texttt{docs/arquitectura/c4-nivel3-componentes.svg}, versionado junto
al DSL fuente.
...
\texttt{docs/adr/README.md}); dos de esos temas están cubiertos por un
ADR cuya numeración interna del repositorio no coincide con la
numeración sugerida por la guía (\texttt{ADR-006}/\texttt{ADR-007}
sugeridos por la guía para acceso a datos y despliegue corresponden a
\texttt{ADR-013} y \texttt{ADR-012} en este repositorio), por
continuidad con la numeración ya evaluada en la Tercera Entrega -- ver
la nota explícita en \texttt{docs/adr/README.md} sobre por qué no se
renumeraron los archivos.
...
\hline
2 & Esquema de autenticación & \texttt{ADR-010} + \texttt{ADR-003} + \texttt{ADR-007} \\
\hline
3 & Gestor de base de datos & \texttt{ADR-011} \\
\hline
4 & Estrategia de caché & \texttt{ADR-008} \\
\hline
5 & Estrategia de despliegue & \texttt{ADR-012} \\
\hline
6 & Estrategia de acceso a datos (CRUD-ORM vs.\ SP) & \texttt{ADR-013} \\
\hline
\end{tabularx}
\end{center}
...
\texttt{ADR-006} (versionado de esquema reproducible, Flyway + snapshot),
...
\hline
Seguridad & Alta & Cookie \texttt{HttpOnly+Secure+SameSite=Strict} para el \texttt{refreshToken} (ADR-007), BCrypt costo 12, blacklist de tokens revocados (ADR-003), respuestas de error RFC~7807 sin fuga de detalles internos. \\
\hline
...
La matriz completa de los 43 requisitos vive en
...
capítulo no reproduce las 43 filas (el detalle completo por requisito
...
CRUD-ORM/SP declarada en ADR-013 se aplica de verdad y no solo en
teoría:
...
\hline
CRUD-ORM (exclusivo, incl.\ combinado con Redis/SMTP/caché en memoria) & 20 & 46{,}5\,\% \\
\hline
Híbrido SP + CRUD-ORM (requisitos transversales) & 6 & 14{,}0\,\% \\
\hline
Procedimiento/función SQL (SP, exclusivo) & 8 & 18{,}6\,\% \\
\hline
No aplica (decisión de arquitectura, configuración o UI sin acceso directo a BD) & 7 & 16{,}3\,\% \\
\hline
Sin clasificar\footnote{\texttt{REQ-F-019} y \texttt{REQ-F-020}: la columna \texttt{tipo\_acceso} de esas dos filas de \texttt{matriz.csv} tiene una coma sin escapar dentro del campo, que rompe el parseo CSV estándar de una columna a la siguiente -- un defecto real y menor del propio archivo de datos, detectado al calcular esta tabla, documentado aquí en vez de omitido u ocultado con una cifra redondeada. No se corrige \texttt{matriz.csv} en esta tarea por estar fuera de su alcance.} (defecto de formato en la matriz, ver nota) & 2 & 4{,}7\,\% \\
\hline
...
De los 43 requisitos, 14 (32{,}6\,\%) involucran al menos un
procedimiento o función SQL (8 exclusivamente, 6 en combinación con
CRUD-ORM), consistente con la justificación de ADR-013: reservar los
