...
(\autoref{fig:der-real}) se generó aplicando, en orden numérico, las
migraciones Flyway de \texttt{database/migrations/} sobre una instancia
PostgreSQL~16 real y limpia. Por legibilidad del documento final y para
cumplir la rúbrica de figuras en inglés, la figura insertada usa aliases
en inglés para las principales áreas del esquema; el PDF técnico con los
identificadores reales de base de datos permanece versionado en
\texttt{docs/diagramas/der-real.pdf}. El archivo histórico
\texttt{docs/diagramas/Diagrama\_ER.png}, correspondiente por error a un
dominio clínico ajeno al proyecto, queda documentado como tal en
\texttt{OBS-21}.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{diagramas/er-english.pdf}
\caption{Entity relationship overview with English labels, generated from
the real PostgreSQL schema categories used by SGB-SaaS. The underlying
database identifiers remain unchanged and are preserved in
\texttt{docs/diagramas/der-real.pdf}.}
\label{fig:der-real}
\end{figure}
