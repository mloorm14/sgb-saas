...
un hallazgo real corregido dentro del mismo ciclo ($66{,}7$\,\%), 1 se
verificó limpio sin hallazgo, y 1 quedó parcialmente cerrado a la espera
de un despliegue público con TLS real -- la auditoría encontró y forzó a
corregir defectos concretos, no fue un ejercicio cosmético.
...
$4{,}7$\,\% híbrido de los 43 requisitos) ni la cobertura de pruebas
($81{,}64$\,\% de líneas), pero tuvo un costo real de implementación: una
migración forzada de \texttt{@Procedure} a \texttt{@Query} nativa por un
defecto documentado de Hibernate con procedimientos multi-\texttt{OUT}.
...
\begin{itemize}[leftmargin=1.2cm]
  \item Un sistema web funcional con 43 requisitos trazados, de los
    cuales el $100$\,\% de los de prioridad \texttt{Must} cuenta con
  ...
  \item Evidencia empírica reproducible y versionada como código: 5
    ...
    delta), cobertura JaCoCo real ($81{,}64$\,\% de líneas, commit
    \texttt{a2c88f8}), y auditoría de accesibilidad/rendimiento con
...
una base de 43 requisitos especificados y trazados, con el $100$\,\% de
los requisitos \texttt{Must} verificados
(\autoref{cap:ingenieria-requisitos}) y $81{,}64$\,\% de cobertura de
línea en 203 pruebas automatizadas del backend
(\autoref{sec:res-cobertura}). De los cinco bloques de evaluación
empírica exigidos por la guía, dos están completos con umbral cumplido
(rendimiento, cobertura), dos están parcialmente cerrados con el
componente faltante explícitamente identificado (seguridad, calidad web),
y uno -- usabilidad -- no se ha ejecutado todavía
