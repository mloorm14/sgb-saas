...
\autoref{cap:introduccion}. Cada bloque sigue el patrón
Goal-Question-Metric de la \autoref{sec:mm-gqm}: pregunta
...
estimada, con el análisis de la \autoref{sec:mm-estadistica} donde
aplica) y una interpretación en prosa. \textbf{No todos los bloques
...
\textbf{Estadística descriptiva (5 corridas agregadas).} (Ver
\autoref{tab:res-perf-descriptivo}.)
...
\textbf{Sobre la corrección por comparaciones múltiples.} Este documento
reporta una única comparación estadística formal (con $p$-valor) en todo
su alcance: el test de Wilcoxon pareado anterior. Las correcciones por
comparaciones múltiples (Bonferroni, Holm-Bonferroni,
Benjamini-Hochberg) existen para controlar la inflación de la tasa de
falsos positivos cuando se evalúan \emph{varias} hipótesis sobre el
mismo conjunto de datos; con $m=1$ comparación, el factor de corrección
de cualquiera de estos métodos es $1$ y el $p$-valor ajustado es
idéntico al original -- no hay ajuste matemático que aplicar. Los demás
bloques de este capítulo (SUS, Lighthouse) reportan estadística
descriptiva (media, mediana, desviación típica, intervalos de
confianza) sin pruebas de hipótesis adicionales, por lo que tampoco
contribuyen comparaciones al conjunto. Se documenta esto explícitamente
en respuesta al comentario del profesor sobre corrección por
comparaciones múltiples, en vez de aplicar una corrección sin sentido
estadístico sobre un único test solo para incluir el término en el
texto.
...
\midrule
A01 -- Control de acceso roto & Hallazgo real encontrado y corregido (rol admin en libros) & \texttt{2026-\allowbreak 07-\allowbreak 30-\allowbreak owasp-\allowbreak a01-\allowbreak control-\allowbreak acceso-\allowbreak roto.md} + \texttt{-fix-rol-admin-\allowbreak libros.md} \\
A02 -- Fallos criptográficos / TLS & Decisión de arquitectura, preparación del backend y verificación de despliegue real \textbf{CERRADAS} (ADR-015, \texttt{forward-headers-strategy}; header \texttt{Strict-Transport-Security} confirmado activo en producción por \texttt{curl}, 18-ago-2026). & \texttt{2026-\allowbreak 08-\allowbreak 10-\allowbreak owasp-\allowbreak a02-\allowbreak fix-tls-\allowbreak transporte.md} \\
A03 -- Inyección & Verificado \textbf{sin hallazgos} (inyección SQL) & \texttt{2026-\allowbreak 07-\allowbreak 30-\allowbreak owasp-\allowbreak a03-\allowbreak inyeccion.md} \\
A05 -- Mala configuración de seguridad & \textbf{Cerrado} con verificación real (CSP, no-root, Swagger 404 en prod); 1 hallazgo real encontrado y corregido en el proceso (bug de 500 en vez de 404) & \texttt{2026-\allowbreak 08-\allowbreak 11-\allowbreak owasp-\allowbreak a05-\allowbreak verificacion-real.md} \\
A07 -- Fallos de identificación y autenticación & Hallazgo real encontrado y corregido (rate limiting de login ausente) & \texttt{2026-\allowbreak 07-\allowbreak 30-\allowbreak owasp-\allowbreak a07-\allowbreak fallo-identificacion-\allowbreak autenticacion.md} + \texttt{-fix-rate-\allowbreak limiting-login.md} \\
A09 -- Fallos de registro y monitoreo & Hallazgo real encontrado y corregido (logging de autenticación insuficiente) & \texttt{2026-\allowbreak 07-\allowbreak 30-\allowbreak owasp-\allowbreak a09-\allowbreak fallo-registro-\allowbreak monitoreo.md} + \texttt{-fix-logging-\allowbreak autenticacion.md} \\
\bottomrule
\end{tabularx}
...
La evolución completa -- pre-merge, post-merge, corrida re-verificada y
cierre -- se resume en la \autoref{tab:res-jacoco}.
...
histórico correcto. El desglose por capa arquitectónica de esa corrida
se muestra en la \autoref{tab:res-jacoco-capas}.
...
Los scores de las 14 corridas se listan en la
\autoref{tab:res-lighthouse}.
...
investiga más a fondo por alcance de esta entrega. El resumen
comparativo por perfil se muestra en la
\autoref{tab:res-lighthouse-perfiles}.
...
\textbf{Datos crudos.} \texttt{docs/mediciones/sus/sus.csv} -- $N=15$
participantes reales (código anonimizado \texttt{P01}--\texttt{P15}),
por encima del mínimo $N\geq15$ que exige la guía, recolectados entre el
2026-08-28 y el 2026-08-30 mediante muestreo por conveniencia dentro de
la comunidad UTEQ. Cada fila incluye los 10 ítems SUS
(\texttt{Q1}--\texttt{Q10}), dos ítems suplementarios (\texttt{I1}:
facilidad de aprendizaje, \texttt{I2}: uso independiente), datos
demográficos (edad, sexo, experiencia web, dispositivo) y un comentario
abierto. Análisis reproducible y ya ejecutado en
\texttt{scripts/sus-analysis.ipynb} (salidas guardadas en el propio
notebook).

\textbf{Estadística descriptiva.} (Ver \autoref{tab:res-sus}.)
...
\midrule
15 & 82,17 & 82,50 & 3,76 & 77,50 & 90,00 \\
\bottomrule
\end{tabular}
\caption{Estadística descriptiva del puntaje SUS agregado (escala 0--100)}
\label{tab:res-sus}
\end{table}

Intervalo de confianza al 95\,\% (t de Student, $df=14$, dado $N<30$):
$[80{,}08,\ 84{,}25]$ (error estándar $0{,}97$).

Clasificación según la escala de adjetivos de Bangor
\citep{brooke1996sus}: \textbf{Excelente} -- el puntaje promedio
(82,2) queda muy por encima del umbral de aceptabilidad estándar de 68
puntos, y el límite inferior del propio IC~95\,\% (80,08) también se
mantiene por encima de ese umbral.

Desglose por ítem (media, escala 1--5, evidencia visual en
\texttt{docs/mediciones/sus/sus\_boxplot.svg} y
\texttt{sus\_items\_breakdown.svg}, reproducidas en la
\autoref{fig:sus-boxplot} y la \autoref{fig:sus-items}): los 5 ítems de
redacción positiva
(Q1, Q3, Q5, Q7, Q9) promedian 4,13, y los 5 de redacción negativa (Q2,
Q4, Q6, Q8, Q10) promedian 1,56 en su escala cruda -- un valor bajo en
los ítems negativos indica desacuerdo con la afirmación negativa, es
decir, percepción favorable, coherente con el resultado agregado. Los
dos ítems suplementarios refuerzan la lectura: I1 (fácil de aprender)
media 4,07 (DT 0,70), I2 (uso independiente) media 4,40 (DT 0,51).

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{mediciones/sus/sus_boxplot.png}
\caption{Distribución del puntaje SUS agregado ($N=15$), diagrama de
caja con el umbral de aceptabilidad (68) marcado como referencia.}
...
\caption{Desglose por ítem SUS (Q1--Q10, escala 1--5) con desviación
estándar -- ítems de redacción positiva en verde, negativa en naranja.
Conversión a PDF del SVG fuente versionado en
\texttt{docs/mediciones/sus/sus\_items\_breakdown.svg} (pdflatex no
...
\textbf{Estado real.} \textbf{Cerrado.} Bloque ejecutado con $N=15$
participantes, por encima del mínimo exigido por la guía, bajo el
protocolo de consentimiento informado y anonimización ya declarado
arriba. Documentado como resuelto en \texttt{OBS-08} de
...
5. Usabilidad (SUS) & $N=15$ (por encima del mínimo $N\geq15$), score 82,17 (IC~95\,\% $[80{,}08, 84{,}25]$), clasificación Excelente & \textbf{Completo} & Sí \\
\bottomrule
\end{tabularx}
...
De los cinco bloques exigidos por la guía, los cinco están completos con
umbral cumplido (rendimiento, seguridad, cobertura, calidad web,
usabilidad). Esta distribución de ``5 de 5 completos'' es el estado real
del proyecto al momento de escribir este capítulo -- alcanzado en el
cierre de la Entrega Final, no desde el inicio: una versión anterior de
este mismo capítulo reportaba honestamente 4 de 5 mientras el bloque de
usabilidad seguía en $N=0$, y se actualiza aquí solo tras confirmar los
datos reales de \texttt{docs/mediciones/sus/sus.csv}.
