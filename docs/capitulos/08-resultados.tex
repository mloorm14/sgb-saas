...
A02 -- Fallos criptográficos / TLS & \textbf{Parcial}: decisión de arquitectura y preparación del backend \textbf{CERRADAS} (ADR-015, \texttt{forward-headers-strategy}); TLS real activo end-to-end \textbf{GAP CONOCIDO, diferido a Entrega Final} & \texttt{2026-08-10-owasp-a02-\allowbreak fix-tls-\allowbreak transporte.md} \\
...
El control restante con estado no binario (A02) no se fuerza dentro de la
misma categoría ``corregido'' ni ``pendiente sin avance'': su decisión de
arquitectura y la preparación del backend están genuinamente cerradas y
documentadas (ADR-015), pero el propio equipo declara explícitamente que
la verificación del header \texttt{Strict-Transport-Security} real
requiere un despliegue público con TLS activo que todavía no existe -- no
se simula ni se asume ese resultado. La auditoría ZAP, un instrumento
adicional exigido por la guía y distinto de la verificación manual
control por control, sigue sin ejecutarse.
...
\midrule
Antes (\texttt{-0300}) & 95 & 95 & 100 & 82 \\
Después (\texttt{-0330}) & \textbf{99} & \textbf{100} & \textbf{100} & \textbf{100} \\
\midrule
Umbral exigido & $\geq$80 & $\geq$90 & $\geq$90 & $\geq$90 \\
\bottomrule
\end{tabular}
\caption{Scores Lighthouse reales, ambas corridas existentes}
\label{tab:res-lighthouse}
\end{table}

\textbf{Estadística descriptiva -- limitación declarada.} Solo existen
...
exige 3 corridas por perfil (móvil + escritorio, 6 en total); con $n=2$
no se calcula media/desviación típica/IC~95\% de forma significativa (2
puntos no estiman una dispersión confiable), así que este capítulo
reporta ambos valores crudos en vez de una estadística agregada que
sugeriría más robustez de la que hay. Este es trabajo en progreso
asignado a Panama según el plan del equipo, no un hallazgo cerrado.

\textbf{Interpretación.} Con los datos disponibles, los 4 umbrales se
cumplen en la corrida más reciente (Performance~99, Accessibility~100,
Best Practices~100, SEO~100), incluyendo SEO, que no cumplía en la
primera corrida (82) y se corrigió agregando una meta descripción y un
\texttt{robots.txt} real. Una nota de honestidad ya documentada en
\texttt{REPORT.md} y que este capítulo preserva: Accessibility subió de
95 a 100 y Performance de 95 a 99 \textbf{sin que se tocara ningún
estilo, color ni optimización de rendimiento} entre ambas corridas --
la explicación más probable es variabilidad normal entre corridas de
Lighthouse (el \emph{audit} \texttt{color-contrast} depende del render
exacto en el momento de la captura), no un efecto causal de las
correcciones de SEO. Atribuir esa mejora a los cambios habría sido una
lectura causal no respaldada por los datos; se documenta como coincidencia
observada, no como resultado. La limitación real de este bloque no es el
resultado en sí -- es que 2 corridas de un solo perfil no bastan para
sostener con la robustez exigida por la guía ninguna de las dos lecturas
(ni ``se cumplen los umbrales'' de forma robusta, ni la hipótesis de
variabilidad run-to-run).
...
\textbf{Estado real.} \textbf{Pendiente de ejecución.} Declarado en
\texttt{OBS-08} de \texttt{docs/observaciones/OBSERVACIONES.md}, asignado
a Panama, bloqueado hasta que el despliegue público esté funcionando
(trabajo en curso por Cajas). El protocolo completo -- plantilla de
consentimiento informado, técnica de muestreo por conveniencia,
anonimización por código de participante -- ya está definido y versionado
(\texttt{docs/etica/consentimientos/plantilla.md},
\autoref{cap:materiales-metodos}), pero no ejecutado. Este capítulo no
reporta ninguna cifra de SUS porque no existe ninguna que reportar --
inventar un rango plausible o una ``estimación'' aquí sería exactamente
el tipo de dato fabricado que el resto de este documento se niega a
producir.
