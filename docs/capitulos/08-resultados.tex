...
sobre la rama \texttt{fix/errores-encontrados}: 524
tests, 0 fallos, 0 errores, 0 \emph{skipped}, \textbf{78,34\,\%} de líneas
(1595/2036) y \textbf{59,48\,\%} de ramas (345/580). Esta es la única cifra vigente de cobertura citada en el
...
\texttt{fix/errores-encontrados} descrita arriba. Los cortes anteriores permanecen en
...
sobre la totalidad del código analizado (48 clases en la corrida
...
\midrule
Lines & 1595/2036 & \textbf{78,34\,\%} & $\geq$70\,\% \textbf{Sí} \\
Branches & 345/580 & \textbf{59,48\,\%} & Sin umbral formal \\
\bottomrule
\end{tabular}
\caption{Cobertura JaCoCo vigente al cierre de la Entrega Final (rama \texttt{fix/errores-encontrados}).}
...
analizables cerró en 2036 líneas tras las exclusiones JaCoCo documentadas, con los commits
...
cobertura global de líneas supera el \textbf{umbral informal de 70\,\%}
fijado por el equipo para la Entrega Final (78,34\,\% al cierre). Las
capas \texttt{controller}, \texttt{security}, \texttt{scheduling} y
\texttt{service} quedan por encima del umbral interno en líneas; las ramas
mejoran a 59,48\,\% y quedan reportadas como métrica complementaria sin
umbral formal de cierre.
...
Service & Sobre umbral & JaCoCo XML & \textbf{Sí} \\
Domain/Entity & --- & Excluido & N/A \\
\bottomrule
\end{tabular}
\caption{Cobertura JaCoCo por capa arquitectónica, medición vigente al cierre (rama \texttt{fix/errores-encontrados})}
\label{tab:res-jacoco-capas-vigente}
\end{table}

\textbf{Interpretación por capa (vigente al cierre, \autoref{tab:res-jacoco-capas-vigente}).} La medición vigente deja \texttt{controller} por encima del umbral interno (95,54\,\% de líneas; 70,41\,\% de ramas) y \texttt{service} cruza el umbral de líneas (73,78\,\%), con ramas en 56,08\,\%. El detalle numérico queda en el XML versionado para evitar mezclar cifras de cierre con cifras históricas dentro del texto defendido.

La capa \texttt{Domain/Entity} (\texttt{com.uteq.backend.entity}) está deliberadamente excluida de la medición JaCoCo, tal como se declara en \texttt{backend-springboot/pom.xml} mediante las exclusiones: \texttt{com/uteq/backend/entity/**} (entidades JPA con getters/setters generados por Lombok, sin lógica de negocio propia), \texttt{com/uteq/backend/dto/**} (DTOs de transporte, solo estructura de datos), \texttt{com/uteq/backend/config/**} (configuración de beans/framework, probada indirectamente por tests de integración), y \texttt{com/uteq/backend/BackendApplication.class} (punto de entrada, sin lógica testable aislada). Estas exclusiones reflejan que dichos paquetes no aportan lógica de negocio medible en términos de cobertura de línea/rama; el código testable con lógica de negocio real reside en \texttt{service}, \texttt{controller}, \texttt{scheduling} y \texttt{security}, que sí son medidos. Con esas capas por encima del umbral de líneas al cierre, el equipo mantiene la exclusión de entidades/DTOs/config como decisión de diseño documentada y razonada.
...
3. Cobertura (JaCoCo) & Medición vigente de cierre: 78,34\,\% de líneas (1595/2036) y 59,48\,\% de ramas (345/580), respaldada por el XML versionado; cumple el objetivo interno de 70\,\% en líneas & \textbf{Completo} & Sí \\
