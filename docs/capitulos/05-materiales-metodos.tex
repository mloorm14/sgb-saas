...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{JaCoCo} (\texttt{jacoco-maven-plugin} en
    ...
    vigente es \textbf{78,34\,\%} de líneas (1595/2036) — medición en fresco sobre
    \texttt{fix/errores-encontrados} con exclusiones documentadas en
    \texttt{backend-springboot/pom.xml} (\autoref{sec:res-cobertura}) —, por encima del
    umbral de líneas. Las mediciones históricas quedan archivadas en
    \texttt{docs/mediciones/jacoco/}, pero no se citan aquí como cifras
    vigentes para evitar ambigüedad: la única cobertura defendida en el
    entregable final es la medición de cierre indicada arriba (ver
...
Los resultados reales obtenidos para M1--M3 están resumidos en
...
(\texttt{k6-run1.json}--\texttt{k6-run5.json}, formato NDJSON de k6) fueron
la fuente de análisis, pero se retiraron del árbol versionado final por
higiene de repositorio; el entregable conserva el resumen reproducible en
\texttt{docs/mediciones/perf/REPORT.md}, las figuras/tablas derivadas y el
script de análisis. El detalle completo -- entorno exacto
