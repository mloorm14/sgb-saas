...
- **Fecha (ISO 8601 UTC)**: 2026-08-11T11:03:53Z
- **Commit**: `14992d6`
...
Usuario A id=`136` (usuarioA.audit.1786446231@sgb-saas.local), Usuario B id=`137` (usuarioB.audit.1786446231@sgb-saas.local).

**A lee los préstamos de B (`GET /api/v1/prestamos/usuario/137`)** — esperado `403`:
```
HTTP_STATUS:403
{"detail":"No tiene permisos para realizar esta acción.","instance":"/api/v1/prestamos/usuario/137","status":403,"title":"Forbidden"}
```

**A lee sus propios préstamos (`GET /api/v1/prestamos/usuario/136`, control)** — esperado `200`:
```
HTTP_STATUS:200
...
```
HTTP_STATUS:201
{"id":138,"nombre":"' OR '1'='1","correo":"usuarioC.audit.1786446231@sgb-saas.local","roles":["LECTOR"]}
```
...
**6 intentos fallidos consecutivos** contra `usuarioA07.audit.1786446231@sgb-saas.local` — esperado `401` en los primeros 5, `429` en el sexto:
```
intento 1: 401
intento 2: 401
intento 3: 401
...
**Verificación de reseteo del contador** (usuario `usuarioReseteo.audit.1786446231@sgb-saas.local`): 2 fallos, luego login correcto.
```
clave Redis: login-attempts:usuarioReseteo.audit.1786446231@sgb-saas.local:172.18.0.1
...
Usuario `usuarioA09.audit.1786446231@sgb-saas.local` (id=`141`): 2 logins fallidos + 1 login exitoso + 1 logout.

**Logs de aplicación (`docker logs sgb_backend`, filtrados por AuthService + correo de esta corrida):**
```
2026-08-11T11:04:09.938Z  WARN 1 --- [backend] [nio-8080-exec-8] com.uteq.backend.service.AuthService     : Login fallido: correo=usuarioA09.audit.1786446231@sgb-saas.local ip=172.18.0.1
2026-08-11T11:04:10.224Z  WARN 1 --- [backend] [nio-8080-exec-2] com.uteq.backend.service.AuthService     : Login fallido: correo=usuarioA09.audit.1786446231@sgb-saas.local ip=172.18.0.1
2026-08-11T11:04:10.537Z  INFO 1 --- [backend] [io-8080-exec-10] com.uteq.backend.service.AuthService     : Login exitoso: sub=141 correo=usuarioA09.audit.1786446231@sgb-saas.local ip=172.18.0.1
2026-08-11T11:04:10.701Z  INFO 1 --- [backend] [nio-8080-exec-4] com.uteq.backend.service.AuthService     : Logout: correo=usuarioA09.audit.1786446231@sgb-saas.local jti=3db89a77-3b8e-4cf4-a453-5d1af46c6f51 ip=172.18.0.1
```
