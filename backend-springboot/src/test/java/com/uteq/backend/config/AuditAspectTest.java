...
class AuditAspectTest {
    ...
    void cablearAspecto() {
        ...
        aspect = new AuditAspect(userRepository);
    ...
    void ejecutaSetConfigWithId() throws Throwable {
        User user = mock(User.class);
        when(user.getId()).thenReturn(7L);
        when(userRepository.findByEmail("auditor@correo.com")).thenReturn(Optional.of(user));
        ...
        Object result = aspect.setCurrentUser(pjp, txLecturaEscritura());
    ...
    void omiteWithoutAutenticacion() throws Throwable {
        when(pjp.proceed()).thenReturn("ok");
